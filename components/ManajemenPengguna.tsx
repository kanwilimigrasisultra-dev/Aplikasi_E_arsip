            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="email" id="email" value={email} onChange={e => setEmail(e.target.value)} required className="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md focus:ring-sky-500 focus:border-sky-500" />
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                     <div>
                        ...
                        <input type="text" id="jabatan" value={jabatan} onChange={e => setJabatan(e.target.value)} required className="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md focus:ring-sky-500 focus:border-sky-500" />
                    </div>
                     <div>
                        <label htmlFor="role" className="block text-sm font-medium text-slate-700">Peran (Role)</label>
                        <select id="role" value={role} onChange={e => setRole(e.target.value as UserRole)} required className="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md focus:ring-sky-500 focus:border-sky-500">
                ...
                <div>
                    ...
                    <select id="unitKerjaId" value={unitKerjaId} onChange={e => setUnitKerjaId(e.target.value)} required disabled={!isSuperAdmin} className="mt-1 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md focus:ring-sky-500 focus:border-sky-500 disabled:bg-slate-200">
                        {isSuperAdmin ? (
            ...
            <div className="bg-white p-6 rounded-xl shadow-md border border-slate-200">
                <div className="flex justify-between items-center mb-4 flex-wrap gap-4">
                    <div className="flex items-center">
                        <UsersIcon className="w-6 h-6 mr-3 text-sky-700" />
                            ...
                            {users.map(user => (
                                <tr key={user.id} className="bg-white border-b hover:bg-slate-50">
                                    ...
                                    <td className="px-6 py-4 space-x-4">
                                        <button onClick={() => handleOpenEditModal(user)} className="font-medium text-sky-600 hover:text-sky-800 transition-colors">
